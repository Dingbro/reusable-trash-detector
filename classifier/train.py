...
def make_parser():
    ...
    args.add_argument("--max_grad_norm", default=2.0, type=float, help="Max gradient norm.")
...
def main(args):
    ...
    #make dataset
    if args.mode == 'multi' or args.mode=='multihead' or args.mode=='multi-timm':
    ...
    elif args.mode == 'multi-timm':
        model = timm.create_model(args.model_name.replace('timm-',''), pretrained=True)
        model.classifier = nn.Linear(in_features=model.classifier.in_features, out_features=args.num_classes, bias=True)
    else:
