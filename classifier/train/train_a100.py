...
from utils.util import get_model_params, rand_bbox, accuracy, set_seed, cutmix_criterion, cutmix_data, mixup_data, mixup_criterion, f1_score_multi, multi_label_acc, f1_loss
...
def main(args):
    ...
    if args.mode == 'multi' or args.mode =='multihead':
        trainset = GarbageDataset(root = args.root, data_file = args.trainfile, transform = train_preprocess, soft_label = args.soft_label, mode='train')
        trainset_after = GarbageDataset(root = args.root, data_file = args.trainfile, transform = train_after_preprocess, soft_label = args.soft_label, mode='train')
        valset = GarbageDataset(root = args.root, data_file = args.evalfile, transform = val_preprocess, soft_label = args.soft_label, mode='eval')
    elif args.mode == 'bbox':
